...
Select c.customer_id,c.last_name,c.first_name, year(s.order_date) as year, month(s.order_date) as month,sum(s.quantity*p.price) as agg_amount 
From ${var:database_name}.customers c join ${var:database_name}.orders s on (c.customer_id = s.customer_id) join ${var:database_name}.products p on (s.product_id=p.product_id)
